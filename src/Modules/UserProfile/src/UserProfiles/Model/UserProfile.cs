...
public record UserProfile : Aggregate<UserProfileId>
{
    ...
    public static UserProfile Create(UserProfileId userProfileId,UserId userId,UserName userName,Name name,Email email,
        bool isDeleted = false)
    {
        var userProfile = new UserProfile{Id = userProfileId,UserId = userId,UserName = userName,Name = name,Email = email,
            IsDeleted = isDeleted};

        var @event = new UserProfileCreatedDomainEvent(userProfile.Id,userProfile.UserId,userProfile.UserName,userProfile.Name,
            userProfile.Email,userProfile.IsDeleted);
